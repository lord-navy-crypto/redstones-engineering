 ...
 *
 * <p>Phase 2 deliberately does not attempt to make an arbitrarily long continuous combinational
 * feedback loop safe by patching an individual block. The production contract is instead explicit:
 * dynamic plant measurement crosses a Sample & Hold boundary before it re-enters PID process value.
 * The long sensor/conditioner return remains live, while PID PV changes only on a commanded sample.
 *
 * <p>The trigger fixture is also topology-safe. With the servo facing NORTH its redstone control
 * faces are SOUTH (command), EAST (brake), and UP (mode). The trigger source is WEST of the servo,
 * so it touches no servo control face and is diagonal from the PID.</p>
 */
    ...
    public static void sampledPidLoopBoundsLongDynamicFeedback(GameTestHelper helper) {
        ...
        helper.runAfterDelay(24, () -> {
            ...
            if (SampleHoldBlock.captureCount(helper.getLevel(), holdWorld) != 0) {
                helper.fail("Sample boundary captured without an explicit trigger edge", sampleHold); return;
            }
            if (helper.getBlockState(sampleHold).getValue(DirectionalSignalBlock.OUTPUT) != 0) {
                helper.fail("Sample boundary did not retain its initial held value before the first sample", sampleHold); return;
            }
            if (ServoActuatorBlock.braking(helper.getLevel(), helper.absolutePos(servo))) {
                helper.fail("Isolated trigger fixture leaked into the servo BRAKE/control topology", servo); return;
            }
            ...
            helper.runAfterDelay(4, () -> {
                ...
                // Re-arm the rising-edge sampler, then force the plant to a materially different demand.
                helper.setBlock(trigger, reference(Direction.SOUTH, 0));
                ...
                helper.runAfterDelay(24, () -> {
                    ...
                    if (ServoActuatorBlock.braking(helper.getLevel(), helper.absolutePos(servo))) {
                        helper.fail("Trigger lane contaminated servo control while the loop was holding", servo); return;
                    }

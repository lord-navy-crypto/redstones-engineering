...
public record TopologyDiagnosticsReport(
    ...
    public static TopologyDiagnosticsReport from(TopologyVisualizationSnapshot snapshot) {
        ...
        for (TopologyFaceSnapshot face : snapshot.faces()) {
            ...
            switch (face.linkStatus()) {
                case OPEN -> open++;
                case DOMAIN_MISMATCH, QUANTITY_MISMATCH, UNIT_MISMATCH, DIRECTION_MISMATCH -> mismatch++;
    ...
    public String summary() {
        ...
        if (mismatchCount > 0) return "TOPOLOGY FAIL | incompatible domain/quantity/unit/direction links=" + mismatchCount;

...
public final class OpticalSystemMenu extends EngineeringDeviceMenu {
    public static final int KIND_EMITTER = 0, KIND_RECEIVER = 1, KIND_METER = 2, KIND_SPLITTER = 3,
            KIND_FILTER = 4, KIND_ATTENUATOR = 5, KIND_FREE_SPACE_TX = 6, KIND_FREE_SPACE_RX = 7;
    public static final int BUTTON_PRIMARY_PREVIOUS = 0, BUTTON_PRIMARY_NEXT = 1,
            BUTTON_SECONDARY_PREVIOUS = 2, BUTTON_SECONDARY_NEXT = 3,
    ...
    @Override protected void refreshAuthoritativeSnapshot() {
        ...
        if (block instanceof OpticalEmitterBlock) {
            ...
            primary.set(m.intensity()); secondary.set(m.channel()); quality.set(m.quality().ordinal()); facing.set(state.getValue(OpticalPowerMeterBlock.FACING).ordinal());
            BlockPos target = blockPos.relative(state.getValue(OpticalPowerMeterBlock.FACING));
            ...
            kind.set(KIND_FILTER); OpticalChannelFilterBlock.FilterEvidence e = OpticalChannelFilterBlock.evidence(level, blockPos, state);
            ...
            kind.set(KIND_FREE_SPACE_RX); Direction in = DirectionalSignalBlock.seriesInputSide(state); Direction out = DirectionalSignalBlock.seriesOutputSide(state);
    ...
    private void captureDomainEndpoints(BlockState state) {
        ...
        inputFacing.set(in.ordinal()); outputFacing.set(out.ordinal()); facing.set(out.ordinal());
    }
    ...
    @Override public boolean clickMenuButton(Player player, int id) {
        ...
        if (block instanceof OpticalEmitterBlock) {
            if (id == BUTTON_PRIMARY_PREVIOUS || id == BUTTON_PRIMARY_NEXT) {
                ...
                int channel = state.getValue(OpticalEmitterBlock.CHANNEL); channel = id == BUTTON_SECONDARY_NEXT ? (channel + 1) % 16 : Math.floorMod(channel - 1, 16); state = state.setValue(OpticalEmitterBlock.CHANNEL, channel);
            } else return false;
            level.setBlock(blockPos, state, Block.UPDATE_CLIENTS); if (level instanceof ServerLevel server) DomainNetwork.recomputeOptical(server, blockPos); changed = true;
        } else if (block instanceof OpticalChannelFilterBlock) {
            if (id == BUTTON_PRIMARY_PREVIOUS || id == BUTTON_PRIMARY_NEXT) {
            ...
            Direction current = state.getValue(OpticalPowerMeterBlock.FACING); Direction[] all = Direction.values(); int delta = id == BUTTON_ROTATE_RIGHT ? 1 : -1;
            level.setBlock(blockPos, state.setValue(OpticalPowerMeterBlock.FACING, all[Math.floorMod(current.ordinal() + delta, all.length)]), Block.UPDATE_CLIENTS); changed = true;
        } else if (block instanceof FreeSpaceOpticalTransmitterBlock transmitter) {
            if (id == BUTTON_SECONDARY_PREVIOUS || id == BUTTON_SECONDARY_NEXT) {
                int channel = state.getValue(FreeSpaceOpticalTransmitterBlock.CHANNEL); channel = id == BUTTON_SECONDARY_NEXT ? (channel + 1) % 4 : Math.floorMod(channel - 1, 4);
                level.setBlock(blockPos, state.setValue(FreeSpaceOpticalTransmitterBlock.CHANNEL, channel), Block.UPDATE_CLIENTS); level.scheduleTick(blockPos, transmitter, 1); changed = true;
            } else changed = routeDomain(id);
        } else if (block instanceof FreeSpaceOpticalReceiverBlock receiver) {
            if (id == BUTTON_SECONDARY_PREVIOUS || id == BUTTON_SECONDARY_NEXT) {
                int channel = state.getValue(FreeSpaceOpticalReceiverBlock.CHANNEL); channel = id == BUTTON_SECONDARY_NEXT ? (channel + 1) % 4 : Math.floorMod(channel - 1, 4);
                level.setBlock(blockPos, state.setValue(FreeSpaceOpticalReceiverBlock.CHANNEL, channel), Block.UPDATE_CLIENTS); level.scheduleTick(blockPos, receiver, 1); changed = true;
    ...
    private boolean routeSplitter(int id) {
        if (id == BUTTON_ROTATE_LEFT || id == BUTTON_ROTATE_RIGHT) return DirectionalDomainBlock.rotateWholeRoute(level, blockPos, id == BUTTON_ROTATE_RIGHT);
        BlockState state = level.getBlockState(blockPos); if (!(state.getBlock() instanceof OpticalSplitterBlock splitter)) return false;
        boolean clockwise = id == BUTTON_INPUT_RIGHT || id == BUTTON_OUTPUT_RIGHT;
        if (id == BUTTON_INPUT_LEFT || id == BUTTON_INPUT_RIGHT) {
            Direction output = DirectionalDomainBlock.seriesOutputSide(state), branchB = DirectionalDomainBlock.leftOf(output), oldInput = DirectionalDomainBlock.seriesInputSide(state), next = rotateHorizontal(oldInput, clockwise);
            ...
            Direction input = DirectionalDomainBlock.seriesInputSide(state), oldOutput = DirectionalDomainBlock.seriesOutputSide(state), next = rotateHorizontal(oldOutput, clockwise);
            for (int i = 0; i < 4 && (next == input || DirectionalDomainBlock.leftOf(next) == input); i++) next = rotateHorizontal(next, clockwise);

...
public final class PneumaticSystemMenu extends EngineeringDeviceMenu {
    ...
    protected void refreshAuthoritativeSnapshot() {
        ...
        if (block instanceof AirCompressorBlock) {
            ...
            long samples = PneumaticFlowMeterBlock.measurement(level, blockPos).sampleCount();
            stateFlag.set((int) Math.min(Integer.MAX_VALUE, samples));
            ...
            upstreamPressure.set(upstream.pressure()); downstreamPressure.set(downstream.pressure());
            upstreamQuality.set(upstream.quality().ordinal()); downstreamQuality.set(downstream.quality().ordinal());
            ...
            kind.set(KIND_CYLINDER); directionalSnapshots(state, cylinder); primary.set(PneumaticCylinderBlock.pressure(level, blockPos)); secondary.set(PneumaticCylinderBlock.position(level, blockPos)); tertiary.set(PneumaticCylinderBlock.target(level, blockPos)); auxiliary.set(PneumaticCylinderBlock.travel(level, blockPos));
        } else kind.set(-1);
    }
    ...
    private void setNodeQuality() {
        ...
        inputQuality.set(observation.quality().ordinal()); outputQuality.set(observation.quality().ordinal());
    }

    private void directionalSnapshots(BlockState state, EngineeringPortProvider provider) {
        Direction out; Direction in;
        if (state.hasProperty(DirectionalDomainBlock.FACING)) { out = DirectionalDomainBlock.seriesOutputSide(state); in = DirectionalDomainBlock.seriesInputSide(state); }
        else { out = DirectionalSignalBlock.seriesOutputSide(state); in = DirectionalSignalBlock.seriesInputSide(state); }
        facing.set(out.ordinal()); inputFacing.set(in.ordinal());
        provider.engineeringSnapshot(level, blockPos, state, in).ifPresent(snapshot -> { primary.set((int) Math.round(snapshot.value())); inputQuality.set(snapshot.quality().ordinal()); });
        provider.engineeringSnapshot(level, blockPos, state, out).ifPresent(snapshot -> { secondary.set((int) Math.round(snapshot.value())); outputQuality.set(snapshot.quality().ordinal()); });
    }
    ...
    public boolean clickMenuButton(Player player, int id) {
        ...
        if (block instanceof PressureRegulatorBlock) {
            if (id == BUTTON_PARAMETER_PREVIOUS || id == BUTTON_PARAMETER_NEXT) {
                int value = state.getValue(PressureRegulatorBlock.SETPOINT); value = id == BUTTON_PARAMETER_NEXT ? value % 4 + 1 : value <= 1 ? 4 : value - 1;
                level.setBlock(blockPos, state.setValue(PressureRegulatorBlock.SETPOINT, value), Block.UPDATE_CLIENTS); if (level instanceof ServerLevel server) PneumaticNetwork.recompute(server, blockPos); changed = true;
            } else changed = rotateDirectional(block, id);
        } else if (block instanceof PneumaticValveBlock) {
            if (id == BUTTON_TOGGLE) { level.setBlock(blockPos, state.setValue(PneumaticValveBlock.OPEN, !state.getValue(PneumaticValveBlock.OPEN)), Block.UPDATE_CLIENTS); if (level instanceof ServerLevel server) PneumaticNetwork.recomputeAround(server, blockPos); changed = true; }
            else changed = rotateDirectional(block, id);
        } else if (block instanceof PneumaticReliefValveBlock) {
            if (id == BUTTON_PARAMETER_PREVIOUS || id == BUTTON_PARAMETER_NEXT) {
                int value = state.getValue(PneumaticReliefValveBlock.SETPOINT); value = id == BUTTON_PARAMETER_NEXT ? value % 4 + 1 : value <= 1 ? 4 : value - 1;
                level.setBlock(blockPos, state.setValue(PneumaticReliefValveBlock.SETPOINT, value), Block.UPDATE_CLIENTS); if (level instanceof ServerLevel server) PneumaticNetwork.recomputeAround(server, blockPos); changed = true;
            } else changed = rotateDirectional(block, id);
        } else if (block instanceof PneumaticReceiverBlock || block instanceof PneumaticCheckValveBlock || block instanceof PneumaticFlowMeterBlock || block instanceof PneumaticProportionalValveBlock || block instanceof PneumaticCylinderBlock) changed = rotateDirectional(block, id);
        else return false;
    ...
    private boolean rotateDirectional(Block block, int id) {
        boolean clockwise; boolean changed;
        if (id == BUTTON_ROTATE_LEFT || id == BUTTON_ROTATE_RIGHT) {
            ...
            else if (block instanceof DirectionalSignalBlock) changed = DirectionalSignalBlock.rotateSeriesAxis(level, blockPos, clockwise); else return false;
            ...
            else if (block instanceof DirectionalSignalBlock) changed = DirectionalSignalBlock.rotateSeriesInput(level, blockPos, clockwise); else return false;
            ...
            else if (block instanceof DirectionalSignalBlock) changed = DirectionalSignalBlock.rotateSeriesOutput(level, blockPos, clockwise); else return false;
        } else return false;
    ...
    public int tertiary() { return tertiary.get(); } public int auxiliary() { return auxiliary.get(); } public int stateFlag() { return stateFlag.get(); }
    ...
    public int upstreamPressure() { return upstreamPressure.get(); } public int downstreamPressure() { return downstreamPressure.get(); }
    public PortQuality upstreamQuality() { return quality(upstreamQuality.get()); } public PortQuality downstreamQuality() { return quality(downstreamQuality.get()); }

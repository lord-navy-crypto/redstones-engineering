...
public final class PneumaticSystemMenu extends EngineeringDeviceMenu {
    ...
    public PneumaticSystemMenu(int containerId, Inventory inventory, RegistryFriendlyByteBuf data) { this(containerId, inventory, data.readBlockPos()); }
    ...
    private static boolean hardFault(PortQuality q){return q==PortQuality.FAULT||q==PortQuality.DOMAIN_MISMATCH||q==PortQuality.TOPOLOGY_ERROR;}
    private void setNodeQuality(){PneumaticObservationSupport.Observation o=PneumaticObservationSupport.observe(level,blockPos);inputQuality.set(o.quality().ordinal());outputQuality.set(o.quality().ordinal());}

    private void directionalSnapshots(BlockState state, EngineeringPortProvider provider){
        Direction out,in;
        if(state.hasProperty(DirectionalDomainBlock.FACING)){out=DirectionalDomainBlock.seriesOutputSide(state);in=DirectionalDomainBlock.seriesInputSide(state);}else{out=DirectionalSignalBlock.seriesOutputSide(state);in=DirectionalSignalBlock.seriesInputSide(state);}
        facing.set(out.ordinal());inputFacing.set(in.ordinal());
        provider.engineeringSnapshot(level,blockPos,state,in).ifPresent(s->{primary.set((int)Math.round(s.value()));inputQuality.set(s.quality().ordinal());});
        provider.engineeringSnapshot(level,blockPos,state,out).ifPresent(s->{secondary.set((int)Math.round(s.value()));outputQuality.set(s.quality().ordinal());});
    }

    @Override public boolean clickMenuButton(Player player,int id){
        if(level.isClientSide)return true;if(!stillValid(player))return false;
        BlockState state=level.getBlockState(blockPos);Block block=state.getBlock();boolean changed=false;
        if(block instanceof PressureRegulatorBlock){
            if(id==BUTTON_PARAMETER_PREVIOUS||id==BUTTON_PARAMETER_NEXT){int value=state.getValue(PressureRegulatorBlock.SETPOINT);value=id==BUTTON_PARAMETER_NEXT?value%4+1:value<=1?4:value-1;level.setBlock(blockPos,state.setValue(PressureRegulatorBlock.SETPOINT,value),Block.UPDATE_CLIENTS);if(level instanceof ServerLevel server)PneumaticNetwork.recompute(server,blockPos);changed=true;}else changed=rotateDirectional(block,id);
        }else if(block instanceof PneumaticValveBlock){
            if(id==BUTTON_TOGGLE){level.setBlock(blockPos,state.setValue(PneumaticValveBlock.OPEN,!state.getValue(PneumaticValveBlock.OPEN)),Block.UPDATE_CLIENTS);if(level instanceof ServerLevel server)PneumaticNetwork.recomputeAround(server,blockPos);changed=true;}else changed=rotateDirectional(block,id);
        }else if(block instanceof PneumaticReliefValveBlock){
            if(id==BUTTON_PARAMETER_PREVIOUS||id==BUTTON_PARAMETER_NEXT){int value=state.getValue(PneumaticReliefValveBlock.SETPOINT);value=id==BUTTON_PARAMETER_NEXT?value%4+1:value<=1?4:value-1;level.setBlock(blockPos,state.setValue(PneumaticReliefValveBlock.SETPOINT,value),Block.UPDATE_CLIENTS);if(level instanceof ServerLevel server)PneumaticNetwork.recomputeAround(server,blockPos);changed=true;}else changed=rotateDirectional(block,id);
        }else if(block instanceof PneumaticReceiverBlock||block instanceof PneumaticCheckValveBlock||block instanceof PneumaticFlowMeterBlock||block instanceof PneumaticProportionalValveBlock||block instanceof PneumaticCylinderBlock)changed=rotateDirectional(block,id);else return false;
    ...
    private boolean rotateDirectional(Block block,int id){
        boolean clockwise,changed;
        if(id==BUTTON_ROTATE_LEFT||id==BUTTON_ROTATE_RIGHT){clockwise=id==BUTTON_ROTATE_RIGHT;if(block instanceof DirectionalDomainBlock)changed=DirectionalDomainBlock.rotateSeriesAxis(level,blockPos,clockwise);else if(block instanceof DirectionalSignalBlock)changed=DirectionalSignalBlock.rotateSeriesAxis(level,blockPos,clockwise);else return false;}
        else if(id==BUTTON_INPUT_LEFT||id==BUTTON_INPUT_RIGHT){clockwise=id==BUTTON_INPUT_RIGHT;if(block instanceof DirectionalDomainBlock)changed=DirectionalDomainBlock.rotateSeriesInput(level,blockPos,clockwise);else if(block instanceof DirectionalSignalBlock)changed=DirectionalSignalBlock.rotateSeriesInput(level,blockPos,clockwise);else return false;}
        else if(id==BUTTON_OUTPUT_LEFT||id==BUTTON_OUTPUT_RIGHT){clockwise=id==BUTTON_OUTPUT_RIGHT;if(block instanceof DirectionalDomainBlock)changed=DirectionalDomainBlock.rotateSeriesOutput(level,blockPos,clockwise);else if(block instanceof DirectionalSignalBlock)changed=DirectionalSignalBlock.rotateSeriesOutput(level,blockPos,clockwise);else return false;}else return false;
        if(changed&&level instanceof ServerLevel server)PneumaticNetwork.recomputeAround(server,blockPos);return changed;
    }

    public int kind(){return kind.get();} public int primary(){return primary.get();} public int secondary(){return secondary.get();} public int tertiary(){return tertiary.get();} public int auxiliary(){return auxiliary.get();} public int stateFlag(){return stateFlag.get();}
    public PortQuality inputQuality(){return quality(inputQuality.get());} public PortQuality outputQuality(){return quality(outputQuality.get());}
    public int upstreamPressure(){return upstreamPressure.get();} public int downstreamPressure(){return downstreamPressure.get();} public PortQuality upstreamQuality(){return quality(upstreamQuality.get());} public PortQuality downstreamQuality(){return quality(downstreamQuality.get());}
    public int cylinderSupply(){return cylinderSupply.get();} public int cylinderPathEdges(){return cylinderPathEdges.get();} public int cylinderObservedLoss(){return cylinderObservedLoss.get();} public int cylinderLineLoss(){return cylinderLineLoss.get();} public int cylinderRestrictionLoss(){return cylinderRestrictionLoss.get();}
    public int cylinderResponsePeriod(){return cylinderResponsePeriod.get();} public int cylinderRemainingTicks(){return cylinderRemainingTicks.get();} public int cylinderVelocity(){return cylinderVelocity.get();} public int cylinderError(){return cylinderError.get();} public int cylinderStallTicks(){return cylinderStallTicks.get();} public int cylinderReversals(){return cylinderReversals.get();} public int cylinderSamples(){return cylinderSamples.get();}
    public CommissioningStatus commissioningStatus(){return CommissioningStatus.fromCode(commissioning.get());}
    private static PortQuality quality(int ordinal){PortQuality[] all=PortQuality.values();return ordinal<0||ordinal>=all.length?PortQuality.NO_SIGNAL:all[ordinal];}
    public Direction outputDirection(){int ordinal=facing.get();Direction[] all=Direction.values();return ordinal<0||ordinal>=all.length?Direction.NORTH:all[ordinal];}
    public Direction inputDirection(){int ordinal=inputFacing.get();Direction[] all=Direction.values();return ordinal<0||ordinal>=all.length?outputDirection().getOpposite():all[ordinal];}

...
public final class PneumaticNetwork {
    ...
    public static Set<BlockPos> collect(Level level, BlockPos start) {
        ...
        while (!queue.isEmpty() && seen.size() < NetworkKernel.MAX_NODES) {
            ...
            for (Direction direction : Direction.values()) {
                ...
                if (level.hasChunkAt(next) && isNode(level, next) && discoveryConnects(level, pos, next) && !seen.contains(next)) queue.addLast(next);
            }
        }
    ...
    private static boolean directionalForward(BlockState state, BlockPos from, BlockPos to) { return to.equals(from.relative(directionalOutput(state))); }
    private static boolean directionalBackwardEntry(BlockState state, BlockPos from, BlockPos to) { return from.equals(to.relative(directionalInput(state))); }
    ...
    private static int localLimit(Level level, BlockPos pos, int pressure) {
        ...
        if (state.getBlock() instanceof PressureRegulatorBlock) pressure = Math.min(pressure, state.getValue(PressureRegulatorBlock.SETPOINT) * 25);
        ...
        if (state.getBlock() instanceof PneumaticReliefValveBlock) {
            ...
            if (pressure > setpoint) {
                ...
                if (level instanceof ServerLevel server) {
                    ...
                    server.sendParticles(ParticleTypes.CLOUD, pos.getX()+0.5, pos.getY()+0.9, pos.getZ()+0.5, count, 0.18,0.08,0.18,0.02);
                }
                pressure = setpoint;
            } else PneumaticReliefValveBlock.clearVenting(level, pos);
        }
    ...
    public static void recompute(ServerLevel level, BlockPos start) {
        ...
        for (BlockPos pos : nodes) {
            var block = level.getBlockState(pos).getBlock();
            if (block instanceof AirCompressorBlock) { if (AirCompressorBlock.commandedPressure(level, pos) > 0) sources++; }
            else if (block instanceof AirReservoirBlock) { if (InformationRuntime.value(level, "air_reservoir", pos) > 0) sources++; }
        }
        NetworkKernel.recordDriverState(level, "pneumatic", sources);

        if (truncated) { failClosedTruncated(level, nodes); return; }
        ...
        while (!queue.isEmpty()) {
            ...
            if (node.predecessor != null) predecessor.put(node.pos, node.predecessor); else predecessor.remove(node.pos);
        ...
        for (BlockPos pos : nodes) {
            ...
            var block = level.getBlockState(pos).getBlock();
            if (block instanceof PneumaticReliefValveBlock && pressure <= 0) PneumaticReliefValveBlock.clearVenting(level,pos);
            int oldPressure=InformationRuntime.value(level,"pneumatic",pos), oldQuality=InformationRuntime.quality(level,"pneumatic",pos);
        ...
        for (BlockPos pos : nodes) {
            ...
            if (state.getBlock() instanceof PneumaticFlowMeterBlock) {
                Direction input=directionalInput(state), output=directionalOutput(state);
                int pin=best.getOrDefault(pos.relative(input),0), pout=best.getOrDefault(pos.relative(output),0), dp=Math.max(0,pin-pout);
                int[] runtime=RuntimeIntStore.get(level,"pneumatic_flow",pos,4);
                runtime[0]=Math.min(100,dp*12); runtime[1]=dp; runtime[2]=pin; runtime[3]=pout;
            }
            if (state.getBlock() instanceof PneumaticCylinderBlock) {
                int supply=bestSupply.getOrDefault(pos,0), actuator=best.getOrDefault(pos,0), edges=pathEdges(predecessor,pos,nodes.size());
                int observedLoss=Math.max(0,supply-actuator), lineLoss=Math.min(supply,edges), restrictionLoss=Math.max(0,observedLoss-lineLoss);
                int[] runtime=RuntimeIntStore.get(level,ACTUATOR_DIAG_KEY,pos,ACTUATOR_DIAG_SIZE);
                runtime[0]=supply; runtime[1]=actuator; runtime[2]=edges; runtime[3]=observedLoss; runtime[4]=lineLoss; runtime[5]=restrictionLoss;
            }
        }
    }

    private static int pathEdges(Map<BlockPos,BlockPos> predecessor, BlockPos end, int maxNodes) {
        int edges=0; BlockPos cursor=end; Set<BlockPos> seen=new HashSet<>();
        while (edges<maxNodes && seen.add(cursor)) {
            BlockPos previous=predecessor.get(cursor); if(previous==null) break;
            edges++; cursor=previous;
        }
        return edges;
    }
    ...
    private static void failClosedTruncated(ServerLevel level, Set<BlockPos> nodes) {
        for (BlockPos pos:nodes) {
            ...
            if(block instanceof PneumaticFlowMeterBlock) Arrays.fill(RuntimeIntStore.get(level,"pneumatic_flow",pos,4),0);
            if(block instanceof PneumaticCylinderBlock) Arrays.fill(RuntimeIntStore.get(level,ACTUATOR_DIAG_KEY,pos,ACTUATOR_DIAG_SIZE),0);
            if(oldPressure!=0||oldQuality!=0||oldValid) level.updateNeighborsAt(pos,block);
        }
    }

    public static boolean truncated(Level level,BlockPos pos){int[] d=RuntimeIntStore.peek(level,DIAG_KEY,pos);return d!=null&&d.length==DIAG_SIZE&&d[0]!=0;}

    public static ActuatorPathEvidence actuatorPathEvidence(Level level, BlockPos pos) {
        int[] r=RuntimeIntStore.peek(level,ACTUATOR_DIAG_KEY,pos);
        if(r==null||r.length<ACTUATOR_DIAG_SIZE) return new ActuatorPathEvidence(0,0,0,0,0,0);
        return new ActuatorPathEvidence(r[0],r[1],r[2],r[3],r[4],r[5]);
    }

    public static void recomputeAround(ServerLevel level,BlockPos changedPos) {
        if(!isNode(level,changedPos)){RuntimeIntStore.remove(level,DIAG_KEY,changedPos);RuntimeIntStore.remove(level,ACTUATOR_DIAG_KEY,changedPos);}
        Set<BlockPos> covered=new HashSet<>();
        if(isNode(level,changedPos)){Set<BlockPos> component=collect(level,changedPos);if(!component.isEmpty()){recompute(level,changedPos);covered.addAll(component);}}
        for(Direction direction:Direction.values()){BlockPos neighbor=changedPos.relative(direction);if(!isNode(level,neighbor)||covered.contains(neighbor))continue;Set<BlockPos> component=collect(level,neighbor);recompute(level,neighbor);covered.addAll(component);}
    }

    public static int pressure(Level level,BlockPos pos) {
        if(level.getBlockState(pos).getBlock() instanceof AirReservoirBlock) return Math.max(InformationRuntime.value(level,"air_reservoir",pos),InformationRuntime.value(level,"pneumatic",pos));
